...
class ZipfianDistribution:
    def __init__(self, zipfianParam, numElements):
        ...
        self.H_s = ZipfianDistribution.computeHarmonic(
            zipfianParam, numElements)
...
def generateDataMilestone4(dataSizeFact, dataSizeDim1, dataSizeDim2, zipfianParam, numDistinctElements):
    ...
    outputFactTable = pd.DataFrame(np.random.randint(
        0, dataSizeFact/5, size=(dataSizeFact, 4)), columns=['col1', 'col2', 'col3', 'col4'])
    ...
    outputFactTable['col4'] = np.random.randint(
        1, dataSizeDim2, size=(dataSizeFact))

    outputDimTable1 = pd.DataFrame(np.random.randint(
        0, dataSizeDim1/5, size=(dataSizeDim1, 3)), columns=['col1', 'col2', 'col3'])
    ...
    outputDimTable1['col2'] = np.random.randint(
        1, dataSizeDim2, size=(dataSizeDim1))

    outputDimTable2 = pd.DataFrame(np.random.randint(
        0, dataSizeDim2/5, size=(dataSizeDim2, 2)), columns=['col1', 'col2'])
    outputDimTable2['col1'] = np.arange(1, dataSizeDim2+1, 1)

    outputFactTable.to_csv(outputFile1, sep=',', index=False,
                           header=header_line_fact, line_terminator='\n')
    outputDimTable1.to_csv(outputFile2, sep=',', index=False,
                           header=header_line_dim1, line_terminator='\n')
    outputDimTable2.to_csv(outputFile3, sep=',', index=False,
                           header=header_line_dim2, line_terminator='\n')
...
def createTest31():
    ...
    output_file.write(
        'load("'+DOCKER_TEST_BASE_DIR+'/data5_dimension1.csv")\n')
    output_file.write('--\n')
    ...
    output_file.write(
        'load("'+DOCKER_TEST_BASE_DIR+'/data5_dimension2.csv")\n')
    output_file.write(
...
def createTest32(factTable, dimTable2, dataSizeFact, dataSizeDim2, selectivityFact, selectivityDim2):
    ...
    dfFactTableMask = (factTable['col2'] < int(
        (dataSizeFact/5) * selectivityFact))
...
def createTest34(factTable, dimTable1, dataSizeFact, dataSizeDim1, selectivityFact, selectivityDim1):
    ...
    # generate expected results
    dfFactTableMask = (factTable['col2'] < int(
        selectivityFact * (dataSizeFact / 5)))
    dfDimTableMask = (dimTable1['col3'] < int(
        (dataSizeDim1/5) * selectivityDim1))
    preJoinFact = factTable[dfFactTableMask]
...
def generateMilestoneFourFiles(dataSizeFact, dataSizeDim1, dataSizeDim2, zipfianParam, numDistinctElements, randomSeed=47):
    ...
    factTable, dimTable1, dimTable2 = generateDataMilestone4(
        dataSizeFact, dataSizeDim1, dataSizeDim2, zipfianParam, numDistinctElements)
    createTest31()
...
def main(argv):
    ...
    generateMilestoneFourFiles(dataSizeFact, dataSizeDim1, dataSizeDim2,
                               zipfianParam, numDistinctElements, randomSeed=randomSeed)
