...
def generateDataMilestone5(dataSize):
    ...
    outputTable.to_csv(outputFile, sep=',', index=False,
...
def createTest38(dataTable):
    ...
    dataTable = dataTable.append(
        {"col1": -1, "col2": -11, "col3": -111, "col4": -1111}, ignore_index=True)
    dataTable = dataTable.append(
        {"col1": -2, "col2": -22, "col3": -222, "col4": -2222}, ignore_index=True)
    dataTable = dataTable.append(
        {"col1": -3, "col2": -33, "col3": -333, "col4": -2222}, ignore_index=True)
    dataTable = dataTable.append(
        {"col1": -4, "col2": -44, "col3": -444, "col4": -2222}, ignore_index=True)
    dataTable = dataTable.append(
        {"col1": -5, "col2": -55, "col3": -555, "col4": -2222}, ignore_index=True)
...
def createTest39(dataTable, approxSelectivity):
    ...
    output_file.write('s2=select(db1.tbl5.col1,{},{})\n'.format(
        selectValLess2, selectValGreater2))
...
def createTest41(dataTable):
    ...
    output_file.write('print(f1)\n')
    # generate expected results
    dfSelectMask = (dataTable['col2'] >= selectValLess) & (
        dataTable['col2'] < selectValGreater)
...
def createRandomUpdates(dataTable, numberOfUpdates, output_file):
    ...
    for i in range(numberOfUpdates):
        ...
        output_file.write(
            '-- UPDATE tbl5 SET col1 = {} WHERE col2 = {};\n'.format(col1Val+1, col2Val))
        output_file.write(
            'u1=select(db1.tbl5.col2,{},{})\n'.format(col2Val, col2Val+1))
        output_file.write(
            'relational_update(db1.tbl5.col1,u1,{})\n'.format(col1Val+1))
        output_file.write('--\n')
...
def createRandomDeletes(dataTable, numberOfUpdates, output_file):
    for i in range(numberOfUpdates):
        ...
        col1Val = dataTable.values[updatePos][0]
        output_file.write(
            '-- DELETE FROM tbl5 WHERE col1 = {};\n'.format(col1Val))
        output_file.write(
            'd1=select(db1.tbl5.col1,{},{})\n'.format(col1Val, col1Val+1))
        output_file.write('relational_delete(db1.tbl5,d1)\n')
        output_file.write('--\n')
...
def createRandomInserts(dataTable, numberOfInserts, output_file):
    for i in range(numberOfInserts):
        ...
        dataTable = dataTable.append(
            {"col1": col1Val, "col2": col2Val, "col3": col3Val, "col4": col4Val}, ignore_index=True)
        output_file.write('--\n')
    return dataTable
