...
def generateDataFileMidwayCheckin():
    ...
    outputTable = pd.DataFrame(
        list(zip(column1, column2)), columns=['col1', 'col2'])
...
def createTestOne():
    ...
    output_file.write(
        'load(\"'+DOCKER_TEST_BASE_DIR+'/data1_generated.csv\")\n')
...
def createTestFour(dataTable):
    ...
    output_file.write(
        'load(\"'+DOCKER_TEST_BASE_DIR+'/data2_generated.csv\")\n')
...
def createTestFive(dataTable, dataSizeTableTwo, approxSelectivity):
    ...
    selectValLess = np.random.randint(
        int(-1 * (dataSizeTableTwo/2)), highestHighVal)
...
def createTestNine(dataTable, dataSizeTableTwo, approxSelectivity):
    ...
    highestHighVal = int((dataSizeTableTwo/2) - offset)
    selectValLess1 = np.random.randint(
        int(-1 * (dataSizeTableTwo/2)), highestHighVal)
    selectValLess2 = np.random.randint(
        int(-1 * (dataSizeTableTwo/2)), highestHighVal)
    ...
    output_file.write('s1=select(db1.tbl2.col1,{},{})\n'.format(
        selectValLess1, selectValGreater1))
